...
const ProjectOverview = () => {
    ...
    const getProjectPriority = (priority: string) => PRIORITYCOLORS[priority as keyof typeof PRIORITYCOLORS] || PRIORITYCOLORS.LOW;
                            ...
                            <Link key={project.id} to={`/projectsDetail?id=${project.id}&tab=tasks`} className="block p-6 hover:bg-zinc-50 dark:hover:bg-zinc-900/50 transition-colors">
                                <div className="flex items-start justify-between mb-3">
                                    ...
                                    <div className="flex items-center gap-2 ml-4">
                                        ...
                                        <div className={`w-2 h-2 rounded-full border-2 ${getProjectPriority(project.priority)}`} />
                                    </div>
                                </div>
