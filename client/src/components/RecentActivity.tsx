...
const RecentActivity = () => {
    ...
    const getStatusColor = (status: string) => STATUSCOLORS[status as keyof typeof STATUSCOLORS] || STATUSCOLORS.TODO;
    const getTypeIcon = (type: string) => TYPEICONS[type as keyof typeof TYPEICONS] || TYPEICONS.OTHER;
            ...
            <div className="p-0">
                {tasks.length === 0 ? (
                    ...
                    <div className="divide-y divide-zinc-200 dark:divide-zinc-800">
                        {tasks.map((task) => {
                            const TypeIcon = getTypeIcon(task.type).icon || Square;
                            const iconColor = getTypeIcon(task.type).color || "text-gray-500 dark:text-gray-400";
