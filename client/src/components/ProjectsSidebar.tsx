...
const ProjectsSidebar = () => {
    ...
    const toggleProject = (id: string) => {
        ...
        const isExpanded = newSet.has(id) ? newSet.delete(id) : newSet.add(id);
        setExpandedProjects(isExpanded ? newSet : new Set());
    };
                        ...
                        {expandedProjects.has(project.id) && (
                            <div className="ml-5 mt-1 space-y-1">
                                {getProjectSubItems(project.id).map((subItem) => {
                                    // Check if the current path matches the sub-item's URL
                                    const isActive =
