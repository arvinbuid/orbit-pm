...
const ProjectCalendar = ({ tasks }: ProjectCalendarProps) => {
    ...
    const getTasksForDate = (date: Date) => tasks.filter((task) => isSameDay(task.due_date, date));

    // 5 upcoming tasks
    const upcomingTasks = tasks
        .filter((task) => task.due_date && !isBefore(task.due_date, today) && task.status !== 'DONE')
        .sort((a, b) => Math.abs(new Date(a.due_date).getTime() - new Date(b.due_date).getTime()))
        .slice(0, 5);
    ...
    const handleMonthChange = (direction: 'next' | 'prev') => setCurrentMonth((prev) => direction === 'next' ? addMonths(prev, 1) : subMonths(prev, 1))
    ...
    return (
        <div className="grid lg:grid-cols-3 gap-6">
            ...
            {/* Tasks for Selected Day User Selected on Calendar */}
                            ...
                            {upcomingTasks.map((task) => (
                                ...
                                >
                                    <div className="flex justify-between items-start text-sm">
                                        ...
                                        <span className={`text-xs px-2 py-0.5 rounded ${TYPECOLORS[task.type as keyof typeof TYPECOLORS]}`}>
                                            {task.type}
                                        </span>
                                    </div>
