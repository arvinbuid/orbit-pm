        ...
        <div>
            ...
            <div className="flex flex-wrap gap-4 mb-4">
                {["status", "type", "priority", "assignee"].map((name) => {
                    ...
                    return (
                        <select
                            ...
                            className="border not-dark:bg-white border-zinc-300 dark:border-zinc-800 outline-none px-3 py-1 rounded text-sm text-zinc-900 dark:text-zinc-200 cursor-pointer"
                        >
                            {options[name as keyof typeof options].map((option, idx) => (
                                <option
                                    key={idx}
                                    value={option.value}
                ...
                {(filters.status || filters.type || filters.priority || filters.assignee) && (
                    <button
                        type="button"
                        onClick={() => setFilters({ status: "", type: "", priority: "", assignee: "" })} className="px-3 py-1 flex items-center gap-2 rounded bg-linear-to-br from-purple-400 to-purple-500 text-zinc-100 dark:text-zinc-200 text-sm transition-colors"
                    >
                                ...
                                <tr>
                                    <th className="pl-2 pr-1">
                                        <input
                                            ...
                                            checked={selectedTasks.length === tasks.length} className="size-3 accent-zinc-600 dark:accent-zinc-500" />
                                    </th>
                            ...
                            <tbody>
                                {filteredTasks.length > 0 ? (
                                    filteredTasks.map((task) => {
                                        const { icon: Icon, color } = TYPEICONS[task.type as keyof typeof TYPEICONS] || {};
                                        const { background, prioritycolor } = PRIORITYTEXTS[task.priority as keyof typeof PRIORITYTEXTS] || {};
                                        ...
                                        return (
                                            ...
                                            >
                                                ...
                                                >
                                                    <select
                                                        name="status"
                                                        onChange={(e) => handleStatusChange(task.id, e.target.value)} value={task.status}
                                                        className="outline-none px-2 pr-4 py-1 rounded text-sm text-zinc-900 dark:text-zinc-200 cursor-pointer"
                                                    >
                                                        <option value="TODO" className="dark:text-zinc-900">To Do</option>
                                                        <option value="IN_PROGRESS" className="dark:text-zinc-900">In Progress</option>
                                                        <option value="DONE" className="dark:text-zinc-900">Done</option>
                                                    </select>
                                                </td>
                                                <td className="px-4 py-2">
                                                    <div className="flex items-center gap-2">
