...
const ProjectAnalytics = ({ project, tasks }: ProjectAnalyticsProps) => {
    const { stats, statusData, typeData, priorityData } = useMemo(() => {
        ...
        tasks.forEach((t) => {
            ...
            typeMap[t.type as keyof typeof typeMap]++;
    ...
    // ref: https://recharts.github.io/en-US/guide/cell/
    const PieShape = (props: PieSectorShapeProps) => <Sector {...props} fill={COLORS[props.index % COLORS.length]} />;
                            ...
                            <div key={p.name} className="space-y-2">
                                <div className="flex justify-between items-center">
                                    <div className="flex items-center gap-2">
                                        <ArrowRightIcon className={`size-3.5 ${PRIORITY_COLORS[p.name as keyof typeof PRIORITY_COLORS]} bg-transparent dark:bg-transparent`} />
                                ...
                                <div className="w-full bg-zinc-300 dark:bg-zinc-800 rounded-full h-1.5">
                                    <div
                                        className={`h-1.5 rounded-full ${PRIORITY_COLORS[p.name as keyof typeof PRIORITY_COLORS]}`}
